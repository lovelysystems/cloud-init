...
def handle(name,cfg,cloud,log,args):
    ...
    defmnts = [ [ "ephemeral0", "/media/ephemeral0", "auto", "defaults", "0", "2" ],
    ...
    for i in range(len(cfgmnt)):
        ...
        for j in range(len(cfgmnt[i])):
            # leave the None flag in the second column alone so this line can
            # be later excluded
            if j == 1 and cfgmnt[i][j] is None:
                continue
    ...
    if needswap:
        ...
        except: log.warn("Failed to enable swap. This is normal on a system without a swap partition configured")
